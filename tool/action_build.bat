...
set "FastDevDir=%AndroidObjDir%\fastdev_files"
...
echo [3/5] Collecting all fast deploy files via MSBuild...
:: Use MSBuild to collect all fast deploy items into a known directory
msbuild "%AndroidProjectDir%\UndertaleModToolAvalonia.Android.csproj" ^
    /t:_CollectFastDevFiles ^
    /p:Configuration=Debug ^
    /p:Platform="Any CPU" ^
    /p:AndroidUseSharedRuntime=true ^
    /p:EmbedAssembliesIntoApk=false ^
    /p:FastDevOutputDir="%FastDevDir%"
if %ERRORLEVEL% neq 0 (
    echo WARNING: Custom target _CollectFastDevFiles not found, falling back to manual collection
    :: Fallback: the __override__ directory from the build should already contain all files
    if not exist "%OverrideDir%" (
        echo ERROR: __override__ directory not found
        exit /b 1
    )
    set "FastDevDir=%OverrideDir%"
)
...
"%ExecutePath%7z.exe" a -tzip "%ExecutePath%assets\genouka_patcher.ext" "%FastDevDir%\.__override__"
if %ERRORLEVEL% neq 0 (
